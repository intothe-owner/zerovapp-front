...
export async function uploadSeniorCenterExcel(
  payload: SeniorCenterImportRequest
): Promise<SeniorCenterImportResponse> {
  const formData = new FormData();
  formData.append("file", payload.file);
  formData.append("programYear", String(payload.programYear));
  formData.append("overwrite", String(payload.overwrite));

  const response = await fetch(`${BACKEND_URL}/senior-import`, {
    method: "POST",
    body: formData,
    credentials: "include",
  });
  ...
  if (!response.ok) {
    throw new Error(data?.message || "경로당 명단 업로드 중 오류가 발생했습니다.");
  }
